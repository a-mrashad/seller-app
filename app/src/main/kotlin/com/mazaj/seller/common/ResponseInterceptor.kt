 ...
 */
@Suppress("all")
class ResponseInterceptor @JvmOverloads constructor(
    private val logger: Logger = Logger.DEFAULT,
    private val parseErrorResponse: (String?) -> (Any)
) : Interceptor {
        ...
        if (logHeaders) {
            ...
            if (!logBody || !response.promisesBody()) {
                ...
                if (!buffer.isProbablyUtf8()) {
                    ...
                    return responseOrThrowIOException(response)
                }
    ...
    private fun responseOrThrowIOException(response: Response): Response {
        if (!response.isSuccessful) throw ErrorResponseException(response, parseErrorResponse)
        return response
    }
